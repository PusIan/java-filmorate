...
public class DBReviewStorageTest {
    ...
    public void shouldDelLikeAfterLike() {
        ...
        reviewStorage.addReviewLikeOrDislike(review.getId(), user.getId(), true);
        assertTrue(reviewStorage.isReviewLiked(review.getId(), user.getId(), true));
        reviewStorage.deleteReviewLikeOrDislike(review.getId(), user.getId(), true);
        assertFalse(reviewStorage.isReviewLiked(review.getId(), user.getId(), true));
    }

    private Review setInputReview() {
        User createdUser = userStorage.create(Fixtures.getUser1());
        Director director = directorStorage.create(Fixtures.getDirector());
